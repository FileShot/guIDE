...
function register(ctx) {
  ...
  async function handleLocalChat(ctx, message, context, helpers) {
    ...
    while (iteration < MAX_AGENTIC_ITERATIONS) {
      ...
      try {
        ...
        try {
          if (nativeFunctions && Object.keys(nativeFunctions).length > 0) {
            ...
            const nativeResult = await llmEngine.generateWithFunctions(
              ...
              { ...(context?.params || {}), maxTokens: effectiveMaxTokens, replaceLastUser: iteration > 1 && continuationCount === 0 },
            ...
            result = await llmEngine.generateStream(currentPrompt, {
              ...
              replaceLastUser: iteration > 1 && continuationCount === 0,
            }, (token) => {
        ...
        if (isContextOverflow && contextRotations < MAX_CONTEXT_ROTATIONS) {
          ...
          try {
            summarizer.markRotation();
            saveRotationCheckpoint(summarizer, mcpToolServer);
            ...
            const progressHints = buildProgressHints(summarizer);
            ...
            const hint = partial
              ...
              : `\nContext was rotated. The user request is: ${message.substring(0, 300)}` + progressHints +
            ...
            finalizeRotation(rollingSummary, sessionStore);
            continue;
          } catch (resetErr) {
        ...
        if (isDisposed && !isFatalNoRecovery) {
          ...
          try {
            ...
            // Second attempt: dispose everything and recreate from scratch
            console.error('[AI Chat] Recovery via resetSession failed:', recoveryErr.message, '— attempting full recreation');
            try {
      ...
      let displayChunk = stripToolFences(newContent);
    ...
    cleanResponse = stripToolFences(cleanResponse);
...
/**
 * Strip tool call fences and inline JSON tool calls from display text.
 * Used in both mid-loop display processing and post-loop cleanup.
 */
function stripToolFences(text) {
  let out = text;
  // Strip ```tool/```tool_call fences
  out = out.replace(/\n?```(?:tool_call|tool)\b[\s\S]*?```\n?/g, '');
  out = out.replace(/\n?```(?:tool_call|tool)\b[\s\S]*$/g, '');
  // Strip ```json fences that contain tool call JSON (not legitimate code blocks)
  out = out.replace(/\n?```json\b([\s\S]*?)```\n?/g, (match, content) => {
  ...
  // Strip XML tool_call tags (Qwen3 and other models use this format)
  out = out.replace(/<tool_calls?>\s*[\s\S]*?<\/tool_calls?>/gi, '');
  out = out.replace(/<\/?tool_calls?>/gi, '');
  // Strip inline JSON tool calls with proper brace matching
  {
    const toolPattern = /\[?\s*\{\s*"tool"\s*:\s*"/g;
    let tm;
    const ranges = [];
    while ((tm = toolPattern.exec(out)) !== null) {
      const braceStart = out.indexOf('{', tm.index);
      let depth = 1, ci = braceStart + 1;
      while (ci < out.length && depth > 0) {
        if (out[ci] === '{') depth++;
    ...
    for (let ri = ranges.length - 1; ri >= 0; ri--) {
      out = out.slice(0, ranges[ri][0]) + out.slice(ranges[ri][1]);
    }
  }
  out = out.replace(/\n{3,}/g, '\n\n');
  return out;
...
/**
 * Build incremental progress hints for context rotation prompts.
 */
function buildProgressHints(summarizer) {
  ...
  return incrementalHint + fileProgressHint;
}

/**
 * Save rotation checkpoint to memory for crash recovery.
 */
function saveRotationCheckpoint(summarizer, mcpToolServer) {
  try {
    const checkpoint = {
    ...
    mcpToolServer?._saveMemory?.('_checkpoint', JSON.stringify(checkpoint));
  } catch (_) {}
}

/**
 * Perform post-rotation bookkeeping: mark rotation on rolling summary, persist state.
 */
function finalizeRotation(rollingSummary, sessionStore) {
  rollingSummary.markRotation();
  sessionStore.saveRollingSummary(rollingSummary);
  sessionStore.flush();
}
