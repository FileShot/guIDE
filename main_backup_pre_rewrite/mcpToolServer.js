...
class MCPToolServer {
  ...
  getToolDefinitions() {
    ...
    this._toolDefsCache = [
      ...
      {
        name: 'write_todos',
        description: 'Create a checklist for complex multi-step coding tasks (e.g. building an app, refactoring code). Do NOT use for simple questions, greetings, or conversation.',
        parameters: {
  ...
  async _writeFile(filePath, content) {
    ...
    try {
      ...
      // Overwrite protection — prevents the model from rewriting files it already created.
      // Two layers:
      // 1. Per-turn tracking: if write_file was already called for this path THIS TURN,
      //    block unconditionally. The model must use append_to_file to add content.
      // 2. Disk check: if file exists on disk with content, block writes that would
      //    produce a shorter file (any length reduction = regression).
      if (!this._filesWrittenThisTurn) this._filesWrittenThisTurn = new Set();
      const normalizedPath = fullPath.replace(/\\/g, '/').toLowerCase();
      if (this._filesWrittenThisTurn.has(normalizedPath)) {
        const existingLines = existingContent ? existingContent.split('\n').length : 0;
        console.log(`[MCP] Overwrite blocked — "${filePath}" was already written this turn (${existingLines} lines on disk). Use append_to_file instead.`);
        return {
          success: false,
          error: `BLOCKED: You already wrote "${filePath}" earlier this turn (${existingLines} lines on disk). To add more content, use append_to_file. To modify specific sections, use edit_file. Do NOT use write_file on files you've already written.`,
          existingLines,
          existingChars: existingContent ? existingContent.length : 0,
        };
      }
      ...
      // Track this file as written this turn for overwrite protection
      this._filesWrittenThisTurn.add(normalizedPath);
  ...
  getCompactToolHint(taskType, options) {
    ...
    hint += '\n### Planning\n';
    hint += '- **write_todos**(items) — Plan complex multi-step coding tasks only (not for greetings/questions).\n';
    hint += '- **update_todo**(index, status) — Mark a task done/in-progress.\n';
    hint += '\nYour browser is REAL Chromium. Never say you can\'t browse. When creating files, use write_file — not code blocks in chat.\n';
    return hint;
  }
